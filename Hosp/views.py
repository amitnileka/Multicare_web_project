	...
	if g == 'Patient':
		if request.method == 'POST':
			...
			if a<=1:
				...
				try:
					Appointment.objects.create(doctorname=doctorname,username=username,patientemail=patientemail,appointmentdate=appointmentdate,appointmenttime=appointmenttime,symptoms=symptoms,status=True,gender=gender)
					error = "no"
	...
	if request.method == 'POST':
		...
		try:
			for i in range(count):
				...
				Medicine.objects.create(appid=pid,medname=medname,medmg=medmg,dose=dose,comment=comment)
			Prescription.objects.create(name=name,age=age,weight=weight,bp=bp,city=city,appid=pid)
